...
template <class T, class U = void> struct traits;
...
public:
    template <class T> static void add_type(const std::string &name);

    template <class T> static std::optional<std::string> get_type();
...
template <class T> void doc_types::add_type(const std::string &name) {
...
template <class T> std::optional<std::string> doc_types::get_type() {
    if (types().count(std::type_index(typeid(T))))
...
// TODO: take care of date and datetime types
// TODO: take care of decimal type

template <> struct traits<bool> {
    static json describe() { return {{"type", "boolean"}}; }
};

template <string_like T> struct traits<T> {
    static json describe() { return {{"type", "string"}}; }
};

template <> struct traits<json> {
    static json describe() { return {{"type", "string"}, {"format", "json"}}; }
};
...
template <class T> struct traits<std::vector<T>> {
    static json describe() {
        json v;
        v["type"] = "array";
        if (doc_types::get_type<T>())
            v["items"]["$ref"] =
                std::string("#/definitions/") + doc_types::get_type<T>().value();
        else
...
template <typename... T> struct traits<model<T...>> {
    static json describe() {
        json v;
        using type = model<T...>;
                ...
                if (ref)
                    ...
                    if (!child.count("$ref")) {
                        ...
                        if constexpr (is_optional_v<type>) {
                            ...
                            if constexpr (scymnus::has_type<constraints::min<nested_type>,
                                                            properties>::value)
                                v["properties"][f.name]["minimum"] =
                                    std::get<constraints::min<nested_type>>(f.properties)
                                        .value;
                            ...
                            if constexpr (has_init<properties>::value) {
                                ...
                                v["properties"][f.name]["default"] =
                                    std::get<idx>(f.properties).value();
                            }

                            if constexpr (scymnus::has_type<constraints::max<nested_type>,
                                                            properties>::value)
                                v["properties"][f.name]["maximum"] =
                                    std::get<constraints::max<nested_type>>(f.properties)
                                        .value;
                        }

                        else {
                            // TODO: check min vs max
                            if constexpr (scymnus::has_type<constraints::min<type>,
                                                            properties>::value)
                                v["properties"][f.name]["minimum"] =
                                    std::get<constraints::min<type>>(f.properties).value;

                            if constexpr (scymnus::has_type<constraints::max<type>,
                                                            properties>::value)
                                v["properties"][f.name]["maximum"] =
                                    std::get<constraints::max<type>>(f.properties).value;
                        }
                        ...
                        if constexpr (has_description<properties>::value) {
                            constexpr int idx =
                                tl::index_if<is_description, properties>::value;
                            v["properties"][f.name]["description"] =
                                std::get<idx>(f.properties).str();
                        } else
                            v["properties"][f.name]["description"] = f.name;
                    }
                }
...
template <class T, class... sl> struct traits<enumeration<T, sl...>> {
    static json describe() {
        ...
        v["description"] = "enum"; // swagger editor is not working correctly if no
            // description is given
        return v;
    }
};

template <int Status, class T, http_content_type ContentType>
struct traits<meta_info<Status, T, ContentType>> {

    static json describe() {
        json v;
        v["description"] = http_status_description<Status>();

        if constexpr (!std::is_same_v<std::remove_cvref_t<T>, no_content>) {
            auto ref = doc_types::get_type<std::decay_t<T>>();
            if (ref)
                v["schema"]["$ref"] = "#/definitions/" + ref.value();

            else {
                v["schema"] = traits<std::remove_cvref_t<T>>::describe();
            }
        }
        return v;
    }
...
template <class T> struct traits<std::map<std::string, T>> {
    static json describe() {
        json v;
        v["type"] = "object";
        auto ref = doc_types::get_type<T>();
