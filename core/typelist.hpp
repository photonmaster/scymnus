...
template <class... T> struct size<typelist<T...>> {
...
namespace detail {
template <class L, class... T> struct push_front;
...
template <class L, class... T> using push_front = detail::push_front<L, T...>;
...
template <template <class...> class F, class L1, class L2> struct transform_2;

template <template <class...> class F, template <class...> class L1,
         template <class...> class L2, class... A1, class... A2>
...
// merge, takes a series of lists and creates one typlist holding the types of
// all of them

namespace detail {

// REF:
// https://stackoverflow.com/questions/53394100/concatenating-tuples-as-types/53398815
template <typename T, typename...> struct merge { using type = T; };

template <template <typename...> class T, typename... T1, typename... T2,
         typename... T3>
struct merge<T<T1...>, T<T2...>, T3...> : public merge<T<T1..., T2...>, T3...> {
};

} // namespace detail

template <class... L> using merge_t = typename detail::merge<L...>::type;
...
template <class L, class A> using append = detail::append<L, A>;
...
template <class A, class L> using prepend = detail::prepend<A, L>;

// remove a type from a type list
namespace detail {

template <class T, class L, class R> struct remove;

template <class T, template <class...> class L, class H, class... R,
         class Result>
struct remove<T, L<H, R...>, Result> {
    using type =
        typename remove<T, L<R...>, typename append<Result, H>::type>::type;
};
...
template <template <class> class P, class L, class R> struct remove_if;

template <template <class> class P, template <class...> class L, class H,
         class... R, class Result>
struct remove_if<P, L<H, R...>, Result> {
    using type = typename remove_if<
        P, L<R...>,
        typename std::conditional<P<H>::value, Result,
                                  typename append<Result, H>::type>::type>::type;
};
...
template <template <class> class P, class L, class R> struct select_if;
...
struct select_if<P, L<H, R...>, Result> {
    using type = typename select_if<
        P, L<R...>,
        typename std::conditional<P<H>::value, typename append<Result, H>::type,
                                  Result>::type>::type;
};
...
template <typename T, typename TL> struct contains_type;

template <typename T, template <class> class Container, typename... Us>
struct contains_type<T, Container<Us...>>
...
template <class C1, class C2, class R> struct intersect;

template <class C1, template <class...> class C2, class H, class... R,
         class Result>
struct intersect<C1, C2<H, R...>, Result> {
    using type = typename intersect<
        C1, C2<R...>,
        typename std::conditional<contains_type<H, C1>::value,
...
template <template <class T> class Pred, typename T, std::size_t pos,
         typename... tail>
struct index_if<Pred, std::tuple<T, tail...>, pos>
    : std::conditional<Pred<T>::value, std::integral_constant<std::size_t, pos>,
                       index_if<Pred, std::tuple<tail...>, pos + 1>>::type {};

template <template <class T> class Pred, std::size_t pos>
struct index_if<Pred, std::tuple<>, pos>
    : std::integral_constant<std::size_t, npos> {};

// append_if

namespace detail {
template <template <class> class P, class L, class R> struct append_if;
...
template <template <class...> class L> struct reverse<L<>> {
    using type = L<>;
};
} // namespace detail

template <class L> using reverse = typename detail::reverse<L>::type;
...
template <template <class> class P, template <class...> class L1, class H1,
         class... R1, template <class...> class L2, class H2, class... R2,
         class Result>
struct normalise<P, L1<H1, R1...>, L2<H2, R2...>, Result> {
    using type = typename std::conditional<
        P<H1>::value,
        typename normalise<P, L1<R1...>, L2<R2...>,
                           typename append<Result, H2>::type>::type,
        typename normalise<P, L1<R1...>, L2<H2, R2...>,
                           typename append<Result, empty_t>::type>::type>::type;
};

// end conditions
template <template <class> class P, template <class...> class L1, class L2,
         class Result>
struct normalise<P, L1<>, L2, Result> {
    using type = Result;
};
template <template <class> class P, template <class...> class L1, class H1,
         class... R1, template <class...> class L2, class Result>
struct normalise<P, L1<H1, R1...>, L2<>, Result> {
    using type = typename normalise<P, L1<R1...>, L2<>,
                                    typename append<Result, empty_t>::type>::type;
};
...
template <template <class...> class L1, class... A1,
         template <class...> class L2, class... A2>
struct is_same<L1<A1...>, L2<A2...>> : std::false_type {};

template <template <class...> class L1, template <class...> class L2,
         class... A>
struct is_same<L1<A...>, L2<A...>> : std::true_type {};

// change the name of the type
// e.g. rename<typelist<int, std::string>, std::tuple> is the type
...
// appends an index
namespace detail {
template <class S, std::size_t I> struct append_index;
...
// add_index_if: given a list and a predicate, if the predicate is true for a
// type in the list the indexed position of the type is appended in the output
// index_sequence
...
template <template <class> class P, template <class...> class L, class H,
         class... R, class Result, std::size_t I>
struct append_index_if<P, L<H, R...>, Result, I> {
    using type = typename append_index_if<
        P, L<R...>,
        typename std::conditional<
            P<H>::value, typename append_index<Result, I>::type, Result>::type,
        I + 1>::type;
};
...
template <template <class> class P, class L, class R = std::index_sequence<>,
         std::size_t I = 0>
using append_index_if = typename detail::append_index_if<P, L, R, I>;

// replace  a type with another one.
namespace detail {
template <class F, class T, class L> struct replace;
...
struct replace<F, T, L<A1, A...>> {
    using type =
        typename prepend<typename replace<F, T, L<A...>>::type, A1>::type;
};
