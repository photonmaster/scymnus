...
template <meta::ct_string Name, class T> class path_param {
    T value;
...
template <typename> struct is_path_param : std::false_type {};

template <meta::ct_string S, typename A>
struct is_path_param<path_param<S, A>> : std::true_type {};

template <class T> constexpr bool is_path_param_v = is_path_param<T>::value;

template <meta::ct_string Name> class segment_param {
    std::string value;
...
public:
...
#ifdef TEST_DEBUG
        std::cout << "move ctor of segment_param called with value: " << value
                  << std::endl;
#endif
    }
    ...
    static json describe() { return {}; }
};

template <typename> struct is_segment_param : std::false_type {};
...
template <typename T>
struct is_path_or_segment_param
    : std::disjunction<is_path_param<T>, is_segment_param<T>> {};

template <class T>
constexpr bool is_path_or_segment_param_v = is_path_or_segment_param<T>::value;

template <meta::ct_string Name, class T> class header_param {
    T value;
...
#ifdef TEST_DEBUG
        std::cout << "move ctor of header_param called with value: " << value
                  << std::endl;
#endif
    }
...
template <typename> struct is_header_param : std::false_type {};
...
// TODO: somehow we must set the mime type that we are expecting
template <meta::ct_string Name, class T> class body_param {
    T value;
...
public:
    ...
    static json describe() {
        ...
        if (doc_types::get_type<dtype>()) {
            v["schema"]["$ref"] =
                std::string("#/definitions/") + doc_types::get_type<dtype>().value();
        } else {
...
template <typename> struct is_body_param : std::false_type {};

template <meta::ct_string Name, typename T>
struct is_body_param<body_param<Name, T>> : std::true_type {};

template <typename T> constexpr bool is_body_param_v = is_body_param<T>::value;

template <meta::ct_string Name, class T> class query_param {
...
template <typename> struct is_query_param : std::false_type {};
