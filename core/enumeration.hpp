...
template <class T, class... sl> struct enumeration {
    static_assert(sizeof...(sl) > 0,
                  "enumeration type must have at least one element");
    
public:
    enumeration() { value_ = data()[0]; }
    ...
    static std::array<T, sizeof...(sl)> &data() {
        static std::array<T, sizeof...(sl)> data_ = {
            boost::lexical_cast<T>(sl::str())...};
        return data_;
    }
